...
[`pipx`](https://pipx.pypa.io/) installs CLI tools in isolated environments and automatically adds them to your `PATH`:
...
If you see `WARNING: The script subs-down-n-sync is installed in '...' which is not on PATH`, add the directory to your `PATH`:
...
After editing the profile file, restart your terminal or run `source ~/.bashrc` (Linux/macOS).
...
When passing a directory, videos that already have a subtitle (`<video>.<lang>.srt`) are skipped by default. Use `--overwrite` / `-o` to re-download from the API. Use `--resync` / `-r` to sync the existing subtitle without hitting the API (useful when the subtitle content is correct but out of sync). Use `--parallel` / `-p` to process up to 2 videos concurrently.
